...
// SetCreatorFeeRate sets the "creator_fee_rate" parameter.
func (inst *CreateAmmConfig) SetCreatorFeeRate(creator_fee_rate uint64) *CreateAmmConfig {
	inst.CreatorFeeRate = &creator_fee_rate
	return inst
}
...
func (inst *CreateAmmConfig) findFindAmmConfigAddress(knownBumpSeed uint8) (pda ag_solanago.PublicKey, bumpSeed uint8, err error) {
	var seeds [][]byte
	// const: amm_config
...
func (inst *CreateAmmConfig) FindAmmConfigAddressWithBumpSeed(bumpSeed uint8) (pda ag_solanago.PublicKey, err error) {
	pda, _, err = inst.findFindAmmConfigAddress(bumpSeed)
	return
}

func (inst *CreateAmmConfig) MustFindAmmConfigAddressWithBumpSeed(bumpSeed uint8) (pda ag_solanago.PublicKey) {
	pda, _, err := inst.findFindAmmConfigAddress(bumpSeed)
	if err != nil {
		panic(err)
	}
...
func (inst *CreateAmmConfig) FindAmmConfigAddress() (pda ag_solanago.PublicKey, bumpSeed uint8, err error) {
	pda, bumpSeed, err = inst.findFindAmmConfigAddress(0)
	return
}

func (inst *CreateAmmConfig) MustFindAmmConfigAddress() (pda ag_solanago.PublicKey) {
...
func (inst *CreateAmmConfig) Validate() error {
	// Check whether all (required) parameters are set:
	{
		...
		if inst.CreatorFeeRate == nil {
			return errors.New("CreatorFeeRate parameter is not set")
		}
	}
			...
			programBranch.Child(ag_format.Instruction("CreateAmmConfig")).
				...
				ParentFunc(func(instructionBranch ag_treeout.Branches) {
					...
					instructionBranch.Child("Params[len=6]").ParentFunc(func(paramsBranch ag_treeout.Branches) {
						...
						paramsBranch.Child(ag_format.Param("   CreatorFeeRate", *inst.CreatorFeeRate))
					})
...
func NewCreateAmmConfigInstruction(
	...
	return NewCreateAmmConfigInstructionBuilder().
		...
		SetCreatorFeeRate(creator_fee_rate).
